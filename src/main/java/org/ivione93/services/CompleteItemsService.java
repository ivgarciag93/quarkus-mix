...
public class CompleteItemsService {
  ...
  public CompletedItemsResponse getCombined() {
    final CompletableFuture<ConfigDto> futureConfig = combinedAsyncCallService.getConfig();
    final CompletableFuture<List<ItemInfo>> futureInfo = combinedAsyncCallService.getItemsInfo();
    final CompletableFuture<List<ItemPrice>> futurePrice = priceAsyncCallService.getItemsPrice();
